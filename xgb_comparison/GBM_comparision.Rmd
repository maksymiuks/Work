...
```
As we can see, models are quite similiar. The biggest difference is residuals distrubution in [0, 0.13] compartment where R `gbm` has biggest mistakes. R is worse in compartment [0.13, 0.25] aswell, but diffence is not as easy to spot as in previous area. 
...
We can see the difference in how our model behaves for different values of `fare`. Response is similiar only in [0,50] compartment.
